        ...
        public virtual void ReadInput()
        {
            ...
            if (distanceToTarget > reachedTargetDistance)
            {
                if (dot > 0)
                {
                    ...
                    _brInp = distanceToTarget < stoppingDistance ? 1f : 0f;
                }
                else
                {
                    var reverseDistance = 5f;
                    _vertInp = distanceToTarget > reverseDistance ? 1f : 0f;
                }

                var angleToDir = Vector3.SignedAngle(transform.forward, dirToMovePosition, Vector3.up);
                _horInp = _config.turnCurve.Evaluate(_controller.DesiredTurning / 90);
                _horInp = angleToDir > 0 ? _horInp : _horInp * -1;
            }
            else
            {
